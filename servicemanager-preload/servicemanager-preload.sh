...
function get_published_apis_payload() {
    ...
    for (( i=0; i<array_length; i++ )); do
        # >&2 echo "Element $i: ${commType_array[$i]} ${resourceName_array[$i]} ${operations_array[$i]} ${uri_array[$i]}"
        ...
        resourceStart="
        {
            ...
            \"Operations\": [
                "
        # >&2 echo "resourceStart ${resourceStart}"
        ...
        # >&2 echo "resourceMid ${resourceMid}"
        ...
        # >&2 echo "resourceEnd ${resourceEnd}"

        resourceItem="${resourceStart}${resourceMid}${resourceEnd}"
        # >&2 echo "resourceItem ${resourceItem}"
    ...
    # >&2 echo "resourceBlock ${resourceBlock}"
...
function publish_service() {
    ...
    echo "$aef_profiles" | jq -c '.[]' | while read -r aef_profile; do
        ...
        # >&2 echo "operationsCsv ${operationsCsv}"
        resourcesAr["Operations"]=$operationsCsv

        payload=$(get_published_apis_payload)
        # >&2 echo "PublishService Payload: $payload"
