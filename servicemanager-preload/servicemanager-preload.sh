...
function find_installed_helm_release() {
    helm_release=""
    >&2 echo "Look for r3-dev-nonrtric release"

    helm status r3-dev-nonrtric -n nonrtric
    if [ $? = 0 ]; then
        helm_release="r3-dev-nonrtric"
    else
        >&2 echo "Look for oran-nonrtric release"
        helm status oran-nonrtric -n nonrtric
        if [ $? = 0 ]; then
            helm_release="oran-nonrtric"
        else
            echo "Failed to find a helm installation"
            exit 1
        fi
    fi
    >&2 echo "found ${helm_release} release"
}

function find_installed_services() {
    find_installed_helm_release
    helm_values=$(helm get values ${helm_release} -n nonrtric -o json)
...
json_config=$(yq eval "$yaml_config" -o=json)
