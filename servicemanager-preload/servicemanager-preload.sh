...
echo "Preloading ServiceManager from $yaml_file"
...
kubectl wait --for=condition=Available -n nonrtric --timeout=120s deploy/capifcore

echo "Waiting for servicemanager deployment"
kubectl wait --for=condition=Available -n nonrtric --timeout=120s deploy/servicemanager

echo "Waiting for kong deployment"
kubectl wait --for=condition=Available -n nonrtric --timeout=120s deploy/oran-nonrtric-kong

publish_services_from_config

echo "Servicemanager preload completed"